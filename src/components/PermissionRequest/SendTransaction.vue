...
<template>
  ...
  >
    ...
    <div v-if="request.params[0]?.to" class="flex justify-between">
      <span class="text-sm">To</span>
      <span class="text-sm">{{ truncateMid(request.params[0]?.to) }}</span>
    </div>
    ...
    <div v-if="request.params[0]?.value" class="flex justify-between">
      <span class="text-sm">Value</span>
      <span class="text-sm">{{
        calculateValue(request.params[0]?.value)
      }}</span>
    </div>
    <div v-if="request.params[0]?.data" class="flex flex-col space-y-2 flex-1">
      <span class="text-sm">Data</span>
      <SignMessageAdvancedInfo :info="request.params[0]?.data" />
    </div>
  </div>
</template>
